...
class PlayerManager{
	...
	public static function create(Player $player): void{
		self::$sessions[$player->getUniqueId()->getBytes()] = $session = new PlayerSession($player);
		$session->handle();
	}
	...
	public static function destroy(Player $player): void{
		...
		if(!isset(self::$sessions[$uuid = $player->getUniqueId()->getBytes()])){
			return;
		}
	...
	public static function get(Player $player): ?PlayerSession{
		return self::$sessions[$player->getUniqueId()->getBytes()] ?? null;
	}

	public static function getNonNull(Player $player): PlayerSession{
		return self::$sessions[$player->getUniqueId()->getBytes()];
	}
